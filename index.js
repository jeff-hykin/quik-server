...
let app = Object.assign(express(), {
    ...
    start: async () => {
        ...
        let pathToHtmlEntrypoint = await app.settings.generateFrontendFiles(app.settings.websiteFile, app.settings.computerGeneratedFolder, frontendCode)
        // 
        // setup bundler
        // 
...
let defaultSettings = {
    ...
    //
        ...
        generateFrontendFiles : async (filePathForWebsiteJavascriptEntrypoint, folderPathForAutoGeneratedFiles, frontendCodeAsString) => {
            // Create the JS file
            let jsLibraryLocation = `${folderPathForAutoGeneratedFiles}/special.js`
            await writeFile(absolutePath(jsLibraryLocation), frontendCodeAsString, err => err && console.log(err))
            // create the html file
            let locationOfHtml = `${folderPathForAutoGeneratedFiles}/.website.html`
            await writeFile(absolutePath(locationOfHtml), `<body></body><script src="../${jsLibraryLocation}"></script><script src="../${filePathForWebsiteJavascriptEntrypoint}"></script>`, err => err && console.log(err))
