...
let app = Object.assign(express(), {
    ...
    start: async () => {
        ...
        let pathToHtmlEntrypoint = await app.settings.generateFrontendFiles(frontendCode)
        // 
        // setup bundler
        // 
        await makeAwaitable(app.settings.systemBundlerSetup(pathToHtmlEntrypoint, absolutePath(app.settings.computerGeneratedFolder), app.settings.bundlerOptions))
...
let defaultSettings = {
    port: 3000,
    host: 'localhost',
    websiteFile: absolutePath("./website.jsx"),
    codeFolder: absolutePath("./code"),
    computerGeneratedFolder: absolutePath("./computerGeneratedCode"),
    bundlerOptions: {},
    // 
    // 
    // runtime setups
    ...
    //
        ...
        generateFrontendFiles : async (frontendCodeAsString) => {
            // get locations
            const pathForAutoGeneratedCode = absolutePath(app.settings.computerGeneratedFolder)
            const filePathForWebsiteJavascriptEntrypoint = absolutePath(app.settings.websiteFile)
            // create the folder if it doesnt exist
            mkdirp(pathForAutoGeneratedCode)
            // Create the JS file
            let jsLibraryLocation = `${pathForAutoGeneratedCode}/special.js`
            await writeFile(jsLibraryLocation, frontendCodeAsString)
            // create the html file
            let locationOfHtml = `${pathForAutoGeneratedCode}/.website.html`
            await writeFile(locationOfHtml, `<body></body><script src="${path.basename(jsLibraryLocation)}"></script><script src="${path.relative(path.dirname(locationOfHtml), filePathForWebsiteJavascriptEntrypoint)}"></script>`)
        ...
        systemBundlerSetup : async (pathToHtmlEntrypoint, pathToAutoGeneratedFolder, bundlerOptionsObj) => {
            ...
            let defaultBundlerSettings = { // see https://parceljs.org/api.html for options
                outDir: absolutePath(`${pathToAutoGeneratedFolder}/dist`),
                cacheDir: absolutePath(`${pathToAutoGeneratedFolder}/.cache`),
                outFile: absolutePath('index.html'),
            }
            let bundler = new Bundler(pathToHtmlEntrypoint, Object.assign(defaultBundlerSettings, bundlerOptionsObj))
...
Object.defineProperty(app, "settings", {
    ...
    set: (newValue) => combinedSettings = Object.assign(combinedSettings, newValue)
})
