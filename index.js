...
let defaultSettings = {
    ...
    //
        ...
        generateFrontendFiles : async (frontendCodeAsString) => {
            // get locations
            let pathForAutoGeneratedCode = app.settings.computerGeneratedFolder
            let filePathForWebsiteJavascriptEntrypoint = app.settings.websiteFile
            let folderPathForAutoGeneratedFiles = app.settings.computerGeneratedFolder
            // create the folder if it doesnt exist
            mkdirp(pathForAutoGeneratedCode)
            // Create the JS file
            let jsLibraryLocation = `${folderPathForAutoGeneratedFiles}/special.js`
            await writeFile(absolutePath(jsLibraryLocation), frontendCodeAsString)
            // create the html file
            let locationOfHtml = `${folderPathForAutoGeneratedFiles}/.website.html`
            await writeFile(absolutePath(locationOfHtml), `<body></body><script src="../${jsLibraryLocation}"></script><script src="../${filePathForWebsiteJavascriptEntrypoint}"></script>`)
        ...
        systemBundlerSetup : async (pathToHtmlEntrypoint, pathToAutoGeneratedFolder, bundlerOptionsObj) => {
            ...
            let defaultBundlerSettings = { // see https://parceljs.org/api.html for options
                outDir: absolutePath(`${combinedSettings.computerGeneratedFolder}/dist`),
